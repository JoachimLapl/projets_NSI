import pygame
from math_utils import hypot, map, sumTuple, Vector, Point, Droite, Circle
...
class MAP:
    ...
    def addWheel(self, wheel):
        ...
        self.wheels += (wheel,)
    def draw(self):
        minX, minY = map(self.findPointCase(Vector.add(self.relative, self.view[0])), lambda e,i : max(0, e))
        maxX, maxY = map(self.findPointCase(Vector.add(self.relative, self.view[1])), lambda e,i : min(e + 1, self.mat_d[i] - 1))
            ...
            for x in range(minX,maxX):
                ...
                for k in range(len(MAP.types), 0, -1):
                    if not every(bl, lambda e,i: e!=k):
                        a, b, c, d = map(bl, lambda e,i: 1 if e and e <=k else 0)
                        path = MAP.marching_squares[a * 8 + b * 4 + c * 2 + d]
                        for points in path:
                            points = map(points, lambda e,j : Vector.add(Vector.subtract(Vector.multiply(Vector.add(e,(x,y)), self.l), self.relative),self.center))
                            pygame.draw.polygon(screen, MAP.types[k-1][1], points)
    def findPointCase(self, p):
        return map(p, lambda e,i : floor(e / self.l))
    def vectorCases(self, point, vector):
        p1, p2 = map((point, Vector.add(point, vector)), lambda e,i : self.findPointCase(e))
        r = ()
...
class Ressort:
    ...
    def display(self, Map=None, color=0, width=1):
        if Map:
            a,b = map((self.p.pos,self.q.pos), lambda e,i : Vector.subtract(Vector.add(Vector.multiply(e, Map.l), Map.relative), Map.center))
